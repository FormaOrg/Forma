...
public interface ProjectCollaboratorRepository extends JpaRepository<ProjectCollaborator, Long> {
    ...
    @EntityGraph(attributePaths = {"project", "user"})
    List<ProjectCollaborator> findAllByUserIdAndStatus(Long userId, CollaboratorStatus status);

    List<ProjectCollaborator> findAllByInviteEmailIgnoreCaseAndStatus(String inviteEmail, CollaboratorStatus status);

    Optional<ProjectCollaborator> findByProjectIdAndUserIdAndStatus(Long projectId, Long userId, CollaboratorStatus status);
